...
namespace source2sdk::server
{
    ...
    public:
        ...
        [[maybe_unused]] std::uint8_t pad_0x204[0x28]; // 0x204
        // metadata: MNetworkEnable
        bool m_bIsRescuing; // 0x22c        
        [[maybe_unused]] std::uint8_t pad_0x22d[0x3]; // 0x22d
        // metadata: MNetworkEnable
        server::WeaponPurchaseTracker_t m_weaponPurchasesThisMatch; // 0x230        
        // metadata: MNetworkEnable
        server::WeaponPurchaseTracker_t m_weaponPurchasesThisRound; // 0x288        
        [[maybe_unused]] std::uint8_t pad_0x2e0[0x8];
    };
    #pragma pack(pop)
    
    // Cannot assert offsets of fields in CCSPlayer_ActionTrackingServices because it is not a standard-layout class
    static_assert(sizeof(CCSPlayer_ActionTrackingServices) == 0x2e8);
};
